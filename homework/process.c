...
/* 注意在 linux 0.11 中的 gcc 不支持 // 的注释方法...  */
/* 内核的标准时间是 jiffy，一个 jiffy 就是一个内部时钟周期
而内部时钟周期是由 100HZ 的频率产生。即一个时钟滴答（tick）为 10ms  */
#define HZ 100
#define PRINT 1
...
void cpuio_bound(int last, int cpu_time, int io_time)
{
    ...
    while (last > 0)
    {
        /* CPU Burst */
        times(&start_time);
        /* tms_utime: the amount of time spent executing your code, or the code in the C library
		 * tms_stime: the amount of time spent in the kernel executing code on your behalf
...
#endif
                while (0 < i) /* 需要等两个子进程，因此循环两次 */
                {                           
                    pid = wait(&stat_val);  /* 等待任意子进程，并将子进程的 exit_code 保存在 stat_val（内核代码见 sys_waitpid()）*/
